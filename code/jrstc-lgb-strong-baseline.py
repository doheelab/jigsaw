...
# ## Load Validation and Test data
df_val = pd.read_csv("../input/jigsaw-toxic-severity-rating/validation_data.csv")
df_less_toxic = df_val.rename(columns={"less_toxic":"text"})
df_more_toxic = df_val.rename(columns={"more_toxic":"text"})


valid_dataset_less = JigsawDataset(df_less_toxic, CONFIG["tokenizer"], max_length=CONFIG["max_length"])
valid_dataset_more = JigsawDataset(df_more_toxic, CONFIG["tokenizer"], max_length=CONFIG["max_length"])
valid_loader_less = DataLoader(
    valid_dataset_less,
    batch_size=CONFIG["test_batch_size"],
    num_workers=2,
    shuffle=False,
